...
class afficherTouitesUtilisateur{
...
HTML;
        ...
        $totalPages = $this->getTotalPages($idutilisateur);
        ...
        if ($page < $totalPages) {
            echo '<a href="dispatcher.php?action=afficherTouitesUtilisateur&id_utilisateur='.$idutilisateur.'&page='.($page + 1).'">Page suivante</a>';
        }
        echo '</div>';
...
HTML;
    ...
    private function getTotalPages(string $idutilisateur) {
        $query = "SELECT COUNT(*) FROM TOUITE WHERE id_utilisateur = :id_utilisateur";
        $stmt = $this->db->prepare($query);
