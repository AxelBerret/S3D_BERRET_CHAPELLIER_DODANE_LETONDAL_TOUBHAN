...
class afficherTouitesTag{
    ...
    function afficherTouitesTag(string $tag){
        ...
        $query = "SELECT TOUITE.*, TAG.libelletag, UTILISATEUR.* FROM TOUITE
                  ...
                  WHERE TAG.libelletag = :tag ORDER BY TOUITE.datePub DESC LIMIT :offset, :touiteParPage";
...
HTML;
        ...
        if ($page > 1) {
            echo '<a href="dispatcher.php?action=afficherTouitesTag&tag='.$tagSansHash.'&page='.($page-1).'">Page précédente</a>';
        }else{
            echo "Il n'y a qu'une seule page.";
        }
        if ($page < $totalPages) {
            echo '<a href="dispatcher.php?action=afficherTouitesTag&tag='.$tagSansHash.'&page='.($page + 1).'">Page suivante</a>';
        }
        echo '</div>';
...
HTML;
    ...
    private function getTotalPages($tag) {
